...
class Interfejs:
    ...
    def rysuj_plansze(self, siatka_gracza):
        ...
        for y in range(siatka_gracza.wysokosc):
            for x in range(siatka_gracza.szerokosc):
    ...
    def rysuj_kafelek(self, rect, wartosc):
        ...
        if wartosc == config.FLAGA:
            pygame.draw.polygon(self.okno, self.kolory['flaga'], [
                (rect.x  + 10, rect.y + 25),
                (rect.x + 10, rect.y + 5),
                (rect.x + 25, rect.y + 15)
            ])
            ...
            kolor_tekstu = self.kolory['cyfry'].get(wartosc, (0, 0, 0))
